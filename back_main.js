...
async function updateWorkPeriods(userId, workPeriods){

  // remove former workPeriods
  const workPeriodQuery = new Parse.Query("work-period");
  workPeriodQuery.equalTo("userId",userId);
  let workPeriodsObjs = await workPeriodQuery.find();
  for(let index=0; index<workPeriodsObjs.length; index++){
    await workPeriodsObjs[index].destroy();
  }

  // add new workPeriods
  for(let i=0; i<workPeriods.length;i++){
    let workPeriod = workPeriods[i];
    var startTime = workPeriod.startTime;
    var endTime = workPeriod.endTime;
    var weekday = workPeriod.weekday;
    const workPeriodObj = new Parse.Object("work-period");
    workPeriodObj.set("userId",userId);
    workPeriodObj.set("startTime",startTime);
    workPeriodObj.set("endTime",endTime);
    workPeriodObj.set("weekday",weekday);
    await workPeriodObj.save();
  }
}

async function checkUserValid(request){
  ...
  let userObj = await query.first();
  if(!userObj){
    return false;
  }
  let tokenDate = userObj.get("tokenDate");
  let curDate = new Date();
  if((curDate - tokenDate)>TOKEN_EXPIRE_TIME*1000){
    return false;
  }
  request.params.userId = userObj.id;
  return true;
}
...
Parse.Cloud.define("updateWorkPeriods", async (request) => {
  let workPeriods = request.params.workPeriods;
  let userId = request.params.userId;

  updateWorkPeriods(userId,workPeriods);
  return {"code":ErrorCode.SUCCESS,  "data":ErrorCode.SuccessStr}; 
...
Parse.Cloud.define("registerUser", async (request) => {
  ...
  let userObject = new Parse.Object("user");
  ...
  userObject.set("mobile", mobile);
  userObject.set("role", role);
  userObject.set("age", age);
  userObject.set("password", password);
  if(age>=16){
    userObject.set("isAbove16", true);
  }else{
    userObject.set("isAbove16", false);
  }

  if(role=="student"){
    let studentProfile = request.params.studentProfile;
    userObject.set("studentProfile", studentProfile);
  }else if(role=="teacher"){
    let teacherProfile = request.params.teacherProfile;
    userObject.set("teacherProfile", teacherProfile);
  }

  let workPeriods = request.params.workPeriods;
  if(workPeriods){
    userObject.set("workPeriods", workPeriods);
  }

  let saveRes = await userObject.save();
  updateWorkPeriods(userId,workPeriods);
...
Parse.Cloud.define("login", async (request) => {
  ...
  let token = UUID.v1();
  userObj.set("token",token);
  let curDate = new Date();
  userObj.set("tokenDate",curDate);
  await userObj.save();
...
Parse.Cloud.define("createCompetition", async (request) => {
  ...
  let musicScore = request.params.musicScore;
  ...
  let precautions = request.params.precautions;
  let accompaniment = request.params.accompaniment;
  ...
  competitionObject.set("precautions", precautions);
  competitionObject.set("accompaniment", accompaniment);
  competitionObject.set("tags", tags);
  
  
  if(musicScore){
    competitionObject.set("musicScore", musicScore);
  }
...
Parse.Cloud.define("competitionRejected", async (request) => {
  ...
  competitionObj.set("status","rejected");
  await competitionObj.save()
  return {"code":ErrorCode.SUCCESS,  "data":ErrorCode.SuccessStr};
});

Parse.Cloud.define("competitionApproved", async (request) => {
  ...
  competitionObj.set("status","approved");
  await competitionObj.save()
  return {"code":ErrorCode.SUCCESS,  "data":ErrorCode.SuccessStr};
});
...
Parse.Cloud.define("inviteCompetitionJudge", async (request) => {
  ...
  let inviterId = request.params.userId;
...
Parse.Cloud.define("competitionJudgeAccepted", async (request) => {
  ...
  let judgeId = request.params.userId;
  const competitionQuery = new Parse.Query("competition");
  let competitionObj = await competitionQuery.get(competitionId);
  if(!competitionObj){
    return {"code":ErrorCode.CompetitionNotExist,  "msg":ErrorCode.CompetitionNotExistStr};
  ...
  // 记录这个时间段该用户已经被占用了
  let startTime = competitionObj.get("datetime");
  let duration = competitionObj.get("duration");
  let endTime = competitionObj.get("datetime");
  let min = endTime.getMinutes();
  endTime.setMinutes(min + duration);
  let engagedTimeObject = new Parse.Object("engaged-time");
  engagedTimeObject.set("userid",judgeId);
  engagedTimeObject.set("startTime",startTime);
  engagedTimeObject.set("endTime",endTime);
  engagedTimeObject.set("type","competitionJudge");
  engagedTimeObject.set("eventId",competitionId);
  await engagedTimeObject.save();
...
Parse.Cloud.define("joinCompetition", async (request) => {
  ...
  let type = request.params.type;
  let userId = request.params.userId;
  if((type!="competitor"&&type!="audience") || !objectId ){
  ...
  if(type=="competitors"){
    let competitionants = competitionObj.get("competitors");
    let index = -1; 
    for(let i = 0; i < competitionants.length; i++){
      if(competitionants[i]==userId){
        index = i;
        break;
      }
    }
    ...
    competitionants.push(userId);
    competitionObj.set("competitors",competitionants);
  } else if(type=="audience"){
    let audiences = competitionObj.get("audiences");
    let index = -1; 
    for(let i = 0; i < audiences.length; i++){
      if(audiences[i]==userId){
        index = i;
        break;
      }
    }
...
Parse.Cloud.define("connectionRequest", async (request) => {
  ...
  let inviter = request.params.userId;
...
Parse.Cloud.define("connectionApproved", async (request) => {
  ...
  const inviterQuery = new Parse.Query("user");
  ...
  const inviteeQuery = new Parse.Query("user");
...
Parse.Cloud.define("approveStudentVerify", async (request) => {
  ...
  const  studentQuery = new Parse.Query("user");
...
Parse.Cloud.define("getAgoraTokenForCompetition", async (request) => {
  ...
  let uid = 2882341276;
  let expirationTimeInSeconds = duration*60;
